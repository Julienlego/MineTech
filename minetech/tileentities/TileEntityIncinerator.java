...
public class TileEntityIncinerator extends TileEntityFurnace{
    ...
    //modify string to change name of item
    ...
    //evey operation generates 200 units of heat
    ...
    @Override
    //method responsible for making the furnace function i.e. smelt the item or not
    //method deals with stacks, not converting one item to another
    public void updateEntity()
    {
        ...
        if(!this.worldObj.isRemote)
        {
            ...
            if(flag != (this.furnaceBurnTime > 0)){
                flag1 = true;
                BlockIncinerator.updateFurnaceBlockState(this.furnaceBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
            }
        }
        if(flag1)
        {
    ...
    private boolean canSmelt(){ 
        ...
        ItemStack itemstack = IncineratorRecipes.smelting().getSmeltingResult(furnaceItemStacks[0].getItem().itemID);
    ...
    //modify method to give 1/6 probability of success
    public void smeltItem()
    {
        int randint = (int)(6.0 * Math.random()) + 1;//possible numbers 1-6
        if(!canSmelt())
        {
            return;
        }
        ItemStack itemstack = IncineratorRecipes.smelting().getSmeltingResult(furnaceItemStacks[0].getItem().itemID);//itemstack is the result of smelting
